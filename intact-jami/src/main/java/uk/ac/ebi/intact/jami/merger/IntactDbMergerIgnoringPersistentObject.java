...
public class IntactDbMergerIgnoringPersistentObject<I,T extends Auditable> extends AbstractMIEnricher<I> implements IntactDbMerger<I,T>{

    private IntactDbSynchronizer<I,T> dbSynchronizer;

    public IntactDbMergerIgnoringPersistentObject(IntactDbSynchronizer<I, T> dbSynchronizer){
    ...
    public T merge(T obj1, T obj2) {
        if (obj1 != null){
            try {
                ...
                if (this.dbSynchronizer.isObjectTransientWithIdentifier(obj1)){
                   this.dbSynchronizer.resetObjectIdentifier(obj1);
                }
                return obj1;
